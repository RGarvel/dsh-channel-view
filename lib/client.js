window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * - 悬停气泡统一：行/入口/rail 提示改用宿主 primitives.Tooltip
		...
		function ChannelPanel(props) {
			...
			const entry = react.createElement("button", {
				...
				...tipProps("Channels（浮层备份入口；标题行旁有正式 tab 时二者共用同一视图）"),
			...
			if (!services.useSessions) {
				return withTip(react.createElement("span", null, entry), "diagnostic: sessions service hook unavailable");
			}
			return react.createElement(react.Fragment, null,
