window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * dsh-channel-view 客户端 v2.2（渠道 tab 精修）。
		 *
		...
		/** 行 → 渠道分类。优先级：投影声明 > subagent 标记 > 未观测。 */
		function channelOf(row) {
			const pv = row.projectionValues || {};
			if (typeof pv.channelSpike === "string" && pv.channelSpike !== "unobserved") {
				return { key: "declared:" + pv.channelSpike, label: `渠道 ${pv.channelSpike}`, sort: 0 };
			}
			if (row.origin === "subagent") {
				return { key: "subagent", label: "子代理 (subagent)", sort: 1 };
			}
			return { key: "unobserved", label: "未观测（冷/未声明）", sort: 2 };
		}
		...
		function TabsIsland({ services, rows, archived, workspacePaths }) {
			...
			react.useEffect(() => {
				...
				const attempt = () => {
					if (disposed) return;
					if (anchors && anchors.tabWrap.isConnected && anchors.viewHost.isConnected) return;
			...
			const tabBtn = (label, target) => react.createElement("button", {
				...
				style: {
					...
					borderRadius: 5, fontSize: 12, lineHeight: "18px",
		...
		function ChannelPanel(props) {
			...
			return react.createElement(react.Fragment, null,
				react.createElement(TabsIsland, { services, rows, archived, workspacePaths }),
