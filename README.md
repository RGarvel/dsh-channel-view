...
- **v2.4 折叠（rail）态收束**：宿主侧边栏折叠时——
  ...
  - 往折叠态仍存在的 `sectionHeader` 首位注入**单个「两 tab 堆叠」图标**（28px 圆底、`currentColor` 描边，与 rail 官方图标同观感）：上下两枚圆角矩形代表「工作区 / Channels」两 tab 收拢，左上/左下各一枚对称指示灯、只亮当前 tab 那盏（左上=工作区、左下=Channels）；点击 = 展开宿主侧栏（转发到宿主 toggle 按钮）并恢复原 tab 选择；
